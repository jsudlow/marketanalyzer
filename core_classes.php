...
class Analyzer {
	...
	public function analyze($ticker_symbol) {
		...
		if($this->check_weekend_and_holiday_dates($date)) {
			echo 'start date has popped for weekend use';
			$date = $this->reverse_date_to_last_trading_day($date->format('Y-m-d'));
		}
		$date = $date->format('Y-m-d');
		
		echo "Analyzing: " . "<b>$ticker_symbol</b>" . 
		"<br /> Analysis Report for <b>" . $ticker_symbol . "</b> for the date of <b>" . $date . "</b><br />";
		...
		if (date('H') <= $this->hour_trading_day_ends) { 
			...
			$this->display_daychart($ticker_symbol, $date);
		}
		...
		$this->display_chart_daterange($ticker_symbol,$sixdaysago,$date);

		//Lets get a trend!
		$this->calculate_trend($ticker_symbol,$sixdaysago,$date);

		//Lets display a chart
		$this->generate_chart($ticker_symbol);
	}
	...
	public function generate_chart($ticker_symbol) {
		echo "
        ...
        // create the chart
			window.chart = new Highcharts.StockChart({
				...
				series : [{
					type : 'ohlc',
					name : '$ticker_symbol Stock Price',
					data : [
					/* May 2006 */
...
public function generate_highstock_data_200_day($ticker_symbol,$date) {
		//Capture the data into the database

		//Construct the 2 dim array of data for the OHLC
